...
| `UniformAInt.lean` | `no_magic_square_of_squares_spaq_center` — **the uniform theorem**: no center root s·pᵃ·q, for every a ≥ 1 at once. One induction on a subsumes Theorems E, F, and G. The class family `UClass` (1+3a classes) telescopes by p²-cancellation (`UClass_step`). Four generic cross-pair cores, six pair dispatchers, and four lone dispatchers hold at every rung. The step routers (`no_assignment_step`, `no_ratio2_step`) take the rung a−1 router as a hypothesis; the rung-1 bases close the induction. The M8ₐ↔M9ₐ ratio cell runs the descent through π^{4a} = (π^{2a})² with base (q, pᵃ), generic in the odd base (`ratio_M8_M9_kill_gen`). |
