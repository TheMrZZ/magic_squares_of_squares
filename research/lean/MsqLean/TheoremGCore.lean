...
/-- z² + 1 = p⁴ is impossible: consecutive squares. -/
private lemma consec_sq_kill (z : ℤ) (h : z ^ 2 + 1 = (p : ℤ) ^ 4) : False := by
  have hp2 : (2 : ℤ) ≤ (p : ℤ) := by exact_mod_cast hp.out.two_le
  have hfact : ((p : ℤ) ^ 2 - z) * ((p : ℤ) ^ 2 + z) = 1 := by linear_combination -h
  rcases Int.eq_one_or_neg_one_of_mul_eq_one' hfact with ⟨h1, h2⟩ | ⟨h1, h2⟩ <;>
    nlinarith

/-- J = 3R₄² − I₄² = 4R₄² − p⁴ is never ±1 (mod 16 with odd witnesses). -/
private lemma J_unit_kill
    (hpodd : p % 2 = 1) (A B : ℤ) (hpAB : A ^ 2 + B ^ 2 = p)
    (J : ℤ) (hJ : J = 3 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2)
    (hJu : J = 1 ∨ J = -1) : False := by
  have hp4c : (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 + (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2 = (p : ℤ) ^ 4 := norm4_coord p A B hpAB
  have hJ4 : J = 4 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (p : ℤ) ^ 4 := by linear_combination hJ - hp4c
  obtain ⟨r, hr⟩ := re4_odd' p hpodd A B hpAB
  obtain ⟨d, hd⟩ := odd_cast p hpodd
...
/-- J = 4R₄² − p⁴ ≡ 3 (mod 4). -/
private lemma J_mod4
    (hpodd : p % 2 = 1) (A B : ℤ) (hpAB : A ^ 2 + B ^ 2 = p)
    (J : ℤ) (hJ : J = 3 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2) : J % 4 = 3 := by
  have hp4c : (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 + (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2 = (p : ℤ) ^ 4 := norm4_coord p A B hpAB
  have hJ4 : J = 4 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (p : ℤ) ^ 4 := by linear_combination hJ - hp4c
  obtain ⟨r, hr⟩ := re4_odd' p hpodd A B hpAB
  obtain ⟨d, hd⟩ := odd_cast p hpodd
...
set_option maxHeartbeats 1600000 in
/-- Mixed-pair global kill: I₁₂X = f·M1 dies through the J ∣ q²
classification — signs by J ≡ 3 (mod 4), units mod 16, and the negative
branches by size against q < p⁴. -/
private lemma mixed12_c1_M1
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (f : ℤ) (hf : f = 1 ∨ f = -1)
    (hIX : (((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = f * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im))) : False := by
  have hf2 : f ^ 2 = 1 := by rcases hf with rfl | rfl <;> norm_num
  have hpP : Prime (p : ℤ) := by
    rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
  have hqP : Prime (q : ℤ) := by
  ...
  obtain ⟨J, hJ⟩ : ∃ J, J = 3 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2 := ⟨_, rfl⟩
  have hI12I4J : (((⟨A, B⟩ : GaussianInt) ^ 12).im) = (((⟨A, B⟩ : GaussianInt) ^ 4).im) * J := by rw [hJ]; linear_combination him12
  have hkey : J * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = f * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2) := by
    have h0 : (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (J * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - f * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2)) = 0 := by
      linear_combination hIX - (((⟨C, D⟩ : GaussianInt) ^ 4).re) * him12 + (((⟨C, D⟩ : GaussianInt) ^ 4).re) * (((⟨A, B⟩ : GaussianInt) ^ 4).im) * hJ
    rcases mul_eq_zero.mp h0 with h | h
    · exact absurd h hI40
    · linarith
  have hpJ : ¬ (p : ℤ) ∣ J := by
    ...
    exact hpI12 (hI12I4J ▸ (hdd.mul_left (((⟨A, B⟩ : GaussianInt) ^ 4).im)))
  have hJq2 : J ∣ (q : ℤ) ^ 2 := by
    have hd1 : J ∣ f * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2) := hkey ▸ ⟨(((⟨C, D⟩ : GaussianInt) ^ 4).re), rfl⟩
    have hd2 : J ∣ (p : ℤ) ^ 4 * (q : ℤ) ^ 2 := by
      have hff : f * (f * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2)) = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 := by
        rcases hf with rfl | rfl <;> ring
      exact hff ▸ hd1.mul_left f
    exact ((((hpP.coprime_iff_not_dvd).mpr hpJ).symm).pow_right).dvd_of_dvd_mul_left hd2
  have hJm4 := J_mod4 p hpodd A B hpAB J hJ
  ...
  have hR42 : (1 : ℤ) ≤ (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 := by
    have hR40 : (((⟨A, B⟩ : GaussianInt) ^ 4).re) ≠ 0 := by
      obtain ⟨r, hr⟩ := re4_odd' p hpodd A B hpAB
      rintro h0; rw [h0] at hr; omega
    rcases lt_or_gt_of_ne hR40 with h | h <;> nlinarith
  have hp2sq : (4 : ℤ) ≤ (p : ℤ) ^ 2 := by nlinarith [hp2le]
  have hp4ge : (16 : ℤ) ≤ (p : ℤ) ^ 4 := by nlinarith [hp2sq]
  obtain ⟨i, hi, hass⟩ := (dvd_prime_pow hqP 2).mp hJq2
  interval_cases i
  · -- unit
    rw [pow_zero] at hass
    exact J_unit_kill p hpodd A B hpAB J hJ
      (Int.isUnit_iff.mp (associated_one_iff_isUnit.mp hass))
  · rcases Int.associated_iff.mp hass with hJ1 | hJ1
    · -- J = q : mod 4
      rw [hJ1] at hJm4
      simp only [pow_one] at hJm4
      omega
    · -- J = −q : size
      have hq' : (q : ℤ) = (p : ℤ) ^ 4 - 4 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 := by
        simp only [pow_one] at hJ1
        linear_combination hJ1 - hJ4
      have hX : (((⟨C, D⟩ : GaussianInt) ^ 4).re) = -(f * ((p : ℤ) ^ 4 * (q : ℤ))) := by
        have h0 : (q : ℤ) * ((((⟨C, D⟩ : GaussianInt) ^ 4).re) + f * ((p : ℤ) ^ 4 * (q : ℤ))) = 0 := by
          simp only [pow_one] at hJ1
          linear_combination -hkey + (((⟨C, D⟩ : GaussianInt) ^ 4).re) * hJ1
        rcases mul_eq_zero.mp h0 with h | h
        · exact absurd h (by positivity)
        · linarith
      have hXsq : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 = (p : ℤ) ^ 8 * (q : ℤ) ^ 2 := by
        rw [hX]; linear_combination (p : ℤ) ^ 8 * (q : ℤ) ^ 2 * hf2
      have hq2pos : (0 : ℤ) < (q : ℤ) ^ 2 := by positivity
      have hp8le : (p : ℤ) ^ 8 ≤ (q : ℤ) ^ 2 := by
        nlinarith [hq4c, hXsq, sq_nonneg (((⟨C, D⟩ : GaussianInt) ^ 4).im), hq2pos]
      have hqle : (q : ℤ) ≤ (p : ℤ) ^ 4 - 4 := by linarith [hq', hR42]
      have hq2b : (q : ℤ) ^ 2 ≤ ((p : ℤ) ^ 4 - 4) ^ 2 := by nlinarith [hqle, hq3]
      nlinarith [hp8le, hq2b, hp4ge]
  · rcases Int.associated_iff.mp hass with hJ1 | hJ1
    · -- J = q² : mod 4
      rw [hJ1] at hJm4
      obtain ⟨u, hu⟩ := odd_cast q hqodd
      have : (q : ℤ) ^ 2 = 4 * (u * u + u) + 1 := by rw [hu]; ring
      rw [this] at hJm4
      omega
    · -- J = −q² : size
      have hq2' : (q : ℤ) ^ 2 = (p : ℤ) ^ 4 - 4 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 := by
        linear_combination hJ1 - hJ4
      have hX : (((⟨C, D⟩ : GaussianInt) ^ 4).re) = -(f * (p : ℤ) ^ 4) := by
        have h0 : (q : ℤ) ^ 2 * ((((⟨C, D⟩ : GaussianInt) ^ 4).re) + f * (p : ℤ) ^ 4) = 0 := by
          linear_combination -hkey + (((⟨C, D⟩ : GaussianInt) ^ 4).re) * hJ1
        rcases mul_eq_zero.mp h0 with h | h
        · exact absurd h (by positivity)
        · linarith
      have hXsq : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 = (p : ℤ) ^ 8 := by
        rw [hX]; linear_combination (p : ℤ) ^ 8 * hf2
      have hp8le : (p : ℤ) ^ 8 ≤ (q : ℤ) ^ 4 := by nlinarith [hq4c, hXsq, sq_nonneg (((⟨C, D⟩ : GaussianInt) ^ 4).im)]
      have hq2le : (q : ℤ) ^ 2 ≤ (p : ℤ) ^ 4 - 4 := by linarith [hq2', hR42]
      have hq4b : (q : ℤ) ^ 4 ≤ ((p : ℤ) ^ 4 - 4) ^ 2 := by
...
/-- Both mixed-pair partners p²-divisible: p² ∣ X and p² ∣ Y force
p⁴ ∣ q⁴. -/
private lemma mixed12_p2p2
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (N1 N2 f g : ℤ)
    (hIX : (((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = f * ((p : ℤ) ^ 2 * N1))
    (hRY : (((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) = g * ((p : ℤ) ^ 2 * N2)) : False := by
  have hpP : Prime (p : ℤ) := by
    rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
  obtain ⟨hpR12, hpI12⟩ := p_not_dvd_re12_im12 p hpodd A B hpAB
  have hq4c : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 + (((⟨C, D⟩ : GaussianInt) ^ 4).im) ^ 2 = (q : ℤ) ^ 4 := norm4_coord q C D hqCD
...
/-- The full mixed-pair core at level 12: I₁₂X = f·Kc1 and R₁₂Y = g·Kc2
with Kc1, Kc2 ∈ M0..M7 is impossible. -/
lemma mixed_pair_core_12
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Kc1 Kc2 f g : ℤ) (hf : f = 1 ∨ f = -1) (hg : g = 1 ∨ g = -1)
    (hc1 : Kc1 = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Kc1 = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Kc1 = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Kc1 = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ Kc1 = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kc1 = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ Kc1 = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kc1 = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))
    (hc2 : Kc2 = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Kc2 = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Kc2 = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Kc2 = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ Kc2 = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kc2 = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ Kc2 = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kc2 = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))
    (hIX : (((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = f * Kc1)
    (hRY : (((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) = g * Kc2) : False := by
  have hsplit : ∀ K : ℤ, (K = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ...
      (∃ N, K = (p : ℤ) ^ 2 * N) ∨ K = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im) := by
    rintro K (rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl)
    · exact Or.inl ⟨(p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im), by ring⟩
    · exact Or.inl ⟨(p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)), by ring⟩
    · exact Or.inl ⟨(q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im), by ring⟩
    · exact Or.inl ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)), by ring⟩
    · exact Or.inl ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)), by ring⟩
    · exact Or.inl ⟨(((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re), by ring⟩
    · exact Or.inl ⟨(((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im), by ring⟩
    · exact Or.inr rfl
  rcases hsplit Kc1 hc1 with ⟨N1, rfl⟩ | rfl
  · rcases hsplit Kc2 hc2 with ⟨N2, rfl⟩ | rfl
    · exact mixed12_p2p2 p q hpodd hqodd hpq A B C D hpAB hqCD N1 N2 f g hIX hRY
    · exact mixed12_c2_M7 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD g hg hRY
  · exact mixed12_c1_M7 p q hpodd hqodd A B C D hpAB hqCD f hf hIX

/-- z² + 1 = w² is impossible for w ≥ 2. -/
private lemma consec_sq_kill_gen (z w : ℤ) (hw : 2 ≤ w) (h : z ^ 2 + 1 = w ^ 2) : False := by
  have hfact : (w - z) * (w + z) = 1 := by linear_combination -h
  rcases Int.eq_one_or_neg_one_of_mul_eq_one' hfact with ⟨h1, h2⟩ | ⟨h1, h2⟩ <;> omega

/-- Shared endgame for the deep two-core cells: p⁴ ∣ 20t²J² with p ∤ t, p ∤ J. -/
private lemma deep12_endgame
    (hpodd : p % 2 = 1)
    (t J : ℤ) (hpt : ¬ (p : ℤ) ∣ t) (hpJ : ¬ (p : ℤ) ∣ J)
    (hd : (p : ℤ) ^ 4 ∣ 20 * t ^ 2 * J ^ 2) : False := by
  have hpP : Prime (p : ℤ) := by
    rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
  have hptJ : ¬ (p : ℤ) ∣ t ^ 2 * J ^ 2 := by
    intro hdd
    rcases hpP.dvd_mul.mp hdd with h | h
    · exact hpt (hpP.dvd_of_dvd_pow h)
    · exact hpJ (hpP.dvd_of_dvd_pow h)
  have hp1 : (p : ℤ) ∣ 20 * (t ^ 2 * J ^ 2) := by
    have h1 := (dvd_pow_self (p : ℤ) (by norm_num : (4 : ℕ) ≠ 0)).trans hd
    rwa [show 20 * t ^ 2 * J ^ 2 = 20 * (t ^ 2 * J ^ 2) from by ring] at h1
  have hp20 : (p : ℤ) ∣ 20 := (hpP.dvd_mul.mp hp1).resolve_right hptJ
  have hp5 : p = 5 := by
    have h20 : p ∣ 20 := by exact_mod_cast hp20
    have hle := Nat.le_of_dvd (by norm_num) h20
    have h2le := hp.out.two_le
    interval_cases p <;> first
      | rfl
      | omega
  ...
  subst hp5
  obtain ⟨k, hk⟩ := hd
  push_cast at hk hptJ
  have h5d : (5 : ℤ) ∣ 4 * (t ^ 2 * J ^ 2) := ⟨25 * k, by linarith [hk]⟩
  rcases (by norm_num : Prime (5 : ℤ)).dvd_mul.mp h5d with h | h
  · norm_num at h
  · exact hptJ (by exact_mod_cast h)

set_option maxHeartbeats 1600000 in
/-- Deep two-core cell: 2R₁₂Y = g·q²I₁₂ with p² ∣ X. -/
private lemma mixed12_two_M7deep
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (g : ℤ) (hg : g = 1 ∨ g = -1)
    (hp2X : (p : ℤ) ^ 2 ∣ (((⟨C, D⟩ : GaussianInt) ^ 4).re))
    (hRY : 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)) = g * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))) : False := by
  have hg2 : g ^ 2 = 1 := by rcases hg with rfl | rfl <;> norm_num
  have hpP : Prime (p : ℤ) := by
    rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
  have hqP : Prime (q : ℤ) := by
  ...
  have hq20 : ((q : ℤ) ^ 2) ≠ 0 :=
    pow_ne_zero _ (Int.natCast_ne_zero.mpr hq.out.pos.ne')
  obtain ⟨t, J, w, hI4t, hJ, ht0, hJodd, hYw, hR12w⟩ :=
    M7_chain p q hpodd hqodd A B C D hpAB hqCD g hg hRY
  obtain ⟨hpR12, hpI12⟩ := p_not_dvd_re12_im12 p hpodd A B hpAB
  have hp12c : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 + (((⟨A, B⟩ : GaussianInt) ^ 12).im) ^ 2 = (p : ℤ) ^ 12 := norm12_coord p A B hpAB
  have hq4c : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 + (((⟨C, D⟩ : GaussianInt) ^ 4).im) ^ 2 = (q : ℤ) ^ 4 := norm4_coord q C D hqCD
  have him12 := im12_eq A B
  have hI12tJ : (((⟨A, B⟩ : GaussianInt) ^ 12).im) = 4 * t * J := by
    linear_combination him12 + (3 * (((⟨A, B⟩ : GaussianInt) ^ 4).re) ^ 2 - (((⟨A, B⟩ : GaussianInt) ^ 4).im) ^ 2) * hI4t - 4 * t * hJ
  have hpt : ¬ (p : ℤ) ∣ t := by
    intro hd
    obtain ⟨c, hc⟩ := hd
    exact hpI12 ⟨4 * c * J, by rw [hI12tJ, hc]; ring⟩
  have hpJ : ¬ (p : ℤ) ∣ J := by
    intro hd
    obtain ⟨c, hc⟩ := hd
    exact hpI12 ⟨4 * t * c, by rw [hI12tJ, hc]; ring⟩
  have hR12q2 : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ∣ (q : ℤ) ^ 2 :=
    ⟨g * w, by linear_combination -g * hR12w - (q : ℤ) ^ 2 * hg2⟩
  have hYsq : (((⟨C, D⟩ : GaussianInt) ^ 4).im) ^ 2 = 4 * t ^ 2 * J ^ 2 * w ^ 2 := by rw [hYw]; ring
  have hp2q : IsCoprime ((p : ℤ) ^ 2) ((q : ℤ)) := by
    refine ((hpP.coprime_iff_not_dvd).mpr ?_).pow_left
    intro hd
    have hnat : p ∣ q := by exact_mod_cast hd
    exact hpq ((Nat.prime_dvd_prime_iff_eq hp.out hq.out).mp hnat)
  obtain ⟨j, hj, hass⟩ := (dvd_prime_pow hqP 2).mp hR12q2
  interval_cases j
  · rw [pow_zero] at hass
    have hR2 : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 = 1 := by
      rcases Int.isUnit_iff.mp (associated_one_iff_isUnit.mp hass) with h1 | h1 <;>
        rw [h1] <;> norm_num
    have hp2le : (2 : ℤ) ≤ (p : ℤ) := by exact_mod_cast hp.out.two_le
    have hp6 : (2 : ℤ) ≤ (p : ℤ) ^ 6 := by
      have h64 := pow_le_pow_left₀ (by norm_num : (0 : ℤ) ≤ 2) hp2le 6
      norm_num at h64
      linarith
    exact consec_sq_kill_gen ((((⟨A, B⟩ : GaussianInt) ^ 12).im)) ((p : ℤ) ^ 6) hp6
      (by linear_combination hp12c - hR2)
  · have hR2 : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 = (q : ℤ) ^ 2 := by
      rcases Int.associated_iff.mp hass with h1 | h1 <;> (rw [h1]; try ring)
    have hw2 : w ^ 2 = (q : ℤ) ^ 2 := by
      have hsq : ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * w) ^ 2 = (g * (q : ℤ) ^ 2) ^ 2 := by rw [hR12w]
      have h0 : (q : ℤ) ^ 2 * (w ^ 2 - (q : ℤ) ^ 2) = 0 := by
        linear_combination hsq - w ^ 2 * hR2 + (q : ℤ) ^ 4 * hg2
      rcases mul_eq_zero.mp h0 with h | h
      · exact absurd h hq20
      · linarith
    have hqX : (q : ℤ) ∣ (((⟨C, D⟩ : GaussianInt) ^ 4).re) := by
      refine hqP.dvd_of_dvd_pow (n := 2) ?_
      exact ⟨(q : ℤ) * ((q : ℤ) ^ 2 - 4 * t ^ 2 * J ^ 2), by
        linear_combination hq4c - hYsq - 4 * t ^ 2 * J ^ 2 * hw2⟩
    obtain ⟨x, hx⟩ := hqX
    have hx2 : x ^ 2 = (q : ℤ) ^ 2 - 4 * t ^ 2 * J ^ 2 := by
      have h0 : (q : ℤ) ^ 2 * (x ^ 2 - ((q : ℤ) ^ 2 - 4 * t ^ 2 * J ^ 2)) = 0 := by
        linear_combination hq4c - hYsq - 4 * t ^ 2 * J ^ 2 * hw2
          - ((((⟨C, D⟩ : GaussianInt) ^ 4).re) + (q : ℤ) * x) * hx
      rcases mul_eq_zero.mp h0 with h | h
      · exact absurd h hq20
      · linarith
    have hp2x : (p : ℤ) ^ 2 ∣ x := by
      refine hp2q.dvd_of_dvd_mul_left ?_
      rwa [← hx]
    obtain ⟨x', hx'⟩ := hp2x
    have hd20 : (p : ℤ) ^ 4 ∣ 20 * t ^ 2 * J ^ 2 := by
      refine ⟨(p : ℤ) ^ 8 - x' ^ 2, ?_⟩
      linear_combination hp12c - hR2 - ((((⟨A, B⟩ : GaussianInt) ^ 12).im) + 4 * t * J) * hI12tJ + hx2
        - (x + (p : ℤ) ^ 2 * x') * hx'
    exact deep12_endgame p hpodd t J hpt hpJ hd20
  · have hR2 : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 = (q : ℤ) ^ 4 := by
      rcases Int.associated_iff.mp hass with h1 | h1 <;> (rw [h1]; try ring)
    have hw2 : w ^ 2 = 1 := by
      have hsq : ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * w) ^ 2 = (g * (q : ℤ) ^ 2) ^ 2 := by rw [hR12w]
      have h0 : (q : ℤ) ^ 4 * (w ^ 2 - 1) = 0 := by
        linear_combination hsq - w ^ 2 * hR2 + (q : ℤ) ^ 4 * hg2
      rcases mul_eq_zero.mp h0 with h | h
    ...
    have hd20 : (p : ℤ) ^ 4 ∣ 20 * t ^ 2 * J ^ 2 := by
      refine ⟨(p : ℤ) ^ 8 - x' ^ 2, ?_⟩
      linear_combination hp12c - hR2 - ((((⟨A, B⟩ : GaussianInt) ^ 12).im) + 4 * t * J) * hI12tJ
        + hq4c - hYsq - 4 * t ^ 2 * J ^ 2 * hw2 - ((((⟨C, D⟩ : GaussianInt) ^ 4).re) + (p : ℤ) ^ 2 * x') * hx'
    exact deep12_endgame p hpodd t J hpt hpJ hd20

/-- Both two-core partners p²-divisible. -/
private lemma mixed12_p2p2_two
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (N1 N2 f g : ℤ)
    (hIX : 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) = f * ((p : ℤ) ^ 2 * N1))
    (hRY : 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)) = g * ((p : ℤ) ^ 2 * N2)) : False := by
  have hpP : Prime (p : ℤ) := by
    rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
  obtain ⟨hpR12, hpI12⟩ := p_not_dvd_re12_im12 p hpodd A B hpAB
  have hq4c : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 + (((⟨C, D⟩ : GaussianInt) ^ 4).im) ^ 2 = (q : ℤ) ^ 4 := norm4_coord q C D hqCD
...
set_option maxHeartbeats 800000 in
/-- The coefficient-2 level-12 core: 2I₁₂X = f·Kc1 ∧ 2R₁₂Y = g·Kc2. -/
lemma mixed_pair_core_12_two
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Kc1 Kc2 f g : ℤ) (hf : f = 1 ∨ f = -1) (hg : g = 1 ∨ g = -1)
    (hc1 : Kc1 = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
    ...
    (hc2 : Kc2 = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ...
      ∨ Kc2 = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kc2 = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))
    (hIX : 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) = f * Kc1)
    (hRY : 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)) = g * Kc2) : False := by
  have hsplit : ∀ K : ℤ, (K = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ K = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ K = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ K = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  rcases hsplit Kc1 hc1 with ⟨N1, rfl⟩ | rfl
  · rcases hsplit Kc2 hc2 with ⟨N2, rfl⟩ | rfl
    · exact mixed12_p2p2_two p q hpodd hqodd hpq A B C D hpAB hqCD N1 N2 f g hIX hRY
    · have hp2X : (p : ℤ) ^ 2 ∣ (((⟨C, D⟩ : GaussianInt) ^ 4).re) := by
        have hpP : Prime (p : ℤ) := by
          rw [Int.prime_iff_natAbs_prime]; simpa using hp.out
        obtain ⟨hpR12, hpI12⟩ := p_not_dvd_re12_im12 p hpodd A B hpAB
        ...
        have hc : IsCoprime ((p : ℤ) ^ 2) (2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im)) :=
          (hc2.mul_right ((hpP.coprime_iff_not_dvd).mpr hpI12)).pow_left
        exact hc.dvd_of_dvd_mul_left ⟨f * N1, by linear_combination hIX⟩
      exact mixed12_two_M7deep p q hpodd hqodd hpq A B C D hpAB hqCD g hg hp2X hRY
  · have hI120 : (((⟨A, B⟩ : GaussianInt) ^ 12).im) ≠ 0 := im12_ne_zero p hpodd A B hpAB
    have h2X : 2 * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = f * (q : ℤ) ^ 2 := by
      have h0 : (((⟨A, B⟩ : GaussianInt) ^ 12).im) * (2 * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - f * (q : ℤ) ^ 2) = 0 := by linear_combination hIX
      rcases mul_eq_zero.mp h0 with h | h
      · exact absurd h hI120
      · linarith
...
set_option maxHeartbeats 800000 in
/-- The M8/M9 pair dispatcher into the level-12 mixed core. -/
lemma dispatch_56G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb e1 e2 e3 e4 : ℤ)
    ...
    (hb : Kb = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ...
      ∨ Kb = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kb = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))
    (hE1 : e3 * M8 A B C D + e4 * M9 A B C D = 2 * e1 * Ka)
    (hE2 : e3 * M8 A B C D - e4 * M9 A B C D = 2 * e2 * Kb) : False := by
  rw [M8_coord, M9_coord] at hE1 hE2
  rcases he3 with rfl | rfl <;> rcases he4 with rfl | rfl
  · exact mixed_pair_core_12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kb e1 e2 he1 he2 ha hb
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Ka e2 e1 he2 he1 hb ha
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Ka (-e2) (-e1)
      (by rcases he2 with rfl | rfl <;> norm_num)
      (by rcases he1 with rfl | rfl <;> norm_num) hb ha
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kb (-e1) (-e2)
      (by rcases he1 with rfl | rfl <;> norm_num)
      (by rcases he2 with rfl | rfl <;> norm_num) ha hb
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])

set_option maxHeartbeats 800000 in
/-- The M8/M9 pair in the (a, b) slots: coefficient-2 core. -/
lemma dispatch_ab56G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    ...
    (hd : Kd = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ...
      ∨ Kd = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ Kd = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im))
    (hE1 : e3 * Kc + e4 * Kd = 2 * e1 * M8 A B C D)
    (hE2 : e3 * Kc - e4 * Kd = 2 * e2 * M9 A B C D) : False := by
  rw [M8_coord] at hE1
  rw [M9_coord] at hE2
  rcases he1 with rfl | rfl <;> rcases he2 with rfl | rfl
  · exact mixed_pair_core_12_two p q hpodd hqodd hpq A B C D hpAB hqCD Kc Kd e3 e4 he3 he4 hc hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12_two p q hpodd hqodd hpq A B C D hpAB hqCD Kd Kc e4 e3 he4 he3 hd hc
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12_two p q hpodd hqodd hpq A B C D hpAB hqCD Kd Kc (-e4) (-e3)
      (by rcases he4 with rfl | rfl <;> norm_num)
      (by rcases he3 with rfl | rfl <;> norm_num) hd hc
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact mixed_pair_core_12_two p q hpodd hqodd hpq A B C D hpAB hqCD Kc Kd (-e3) (-e4)
      (by rcases he3 with rfl | rfl <;> norm_num)
      (by rcases he4 with rfl | rfl <;> norm_num) hc hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])

set_option maxHeartbeats 1600000 in
lemma dispatch_ac12G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    (he3 : e3 = 1 ∨ e3 = -1) (he4 : e4 = 1 ∨ e4 = -1)
    (hac : (Ka = M8 A B C D ∧ Kc = M9 A B C D) ∨ (Ka = M9 A B C D ∧ Kc = M8 A B C D))
    (hb : Kb = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Kb = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Kb = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Kb = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  rcases hac with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩ <;>
    simp only [M8_coord, M9_coord] at hE1 hE2 <;>
    rcases he1 with rfl | rfl <;> rcases he3 with rfl | rfl
  -- Ka = L5, Kc = L6
  · -- (e1, e3) = (1, 1): T-form, f = −e2, g = e4, e = 1
    exact cross_pair_core_T12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kd (-e2) e4 1
      (by rcases he2 with rfl | rfl <;> norm_num) he4 (Or.inl rfl) hb hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · -- (1, −1): S-form, f = −e2, g = e4, e = 1
    exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kd (-e2) e4 1
      (by rcases he2 with rfl | rfl <;> norm_num) he4 (Or.inl rfl) hb hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · -- (−1, 1): S-form, f = e2, g = −e4, e = 1
    exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kd e2 (-e4) 1
...
lemma dispatch_ad12G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    (he3 : e3 = 1 ∨ e3 = -1) (he4 : e4 = 1 ∨ e4 = -1)
    (had : (Ka = M8 A B C D ∧ Kd = M9 A B C D) ∨ (Ka = M9 A B C D ∧ Kd = M8 A B C D))
    (hb : Kb = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Kb = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Kb = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Kb = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  · exact cross_pair_core_T12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kc e2 e3 1
      he2 he3 (Or.inl rfl) hb hc (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kc e2 e3 1
      he2 he3 (Or.inl rfl) hb hc (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Kb Kc (-e2) (-e3) 1
      (by rcases he2 with rfl | rfl <;> norm_num)
...
lemma dispatch_bc12G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    (he3 : e3 = 1 ∨ e3 = -1) (he4 : e4 = 1 ∨ e4 = -1)
    (hbc : (Kb = M8 A B C D ∧ Kc = M9 A B C D) ∨ (Kb = M9 A B C D ∧ Kc = M8 A B C D))
    (ha : Ka = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Ka = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Ka = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Ka = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  · exact cross_pair_core_T12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kd (-e1) (-e4) 1
      (by rcases he1 with rfl | rfl <;> norm_num)
      (by rcases he4 with rfl | rfl <;> norm_num) (Or.inl rfl) ha hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kd (-e1) (-e4) 1
      (by rcases he1 with rfl | rfl <;> norm_num)
      (by rcases he4 with rfl | rfl <;> norm_num) (Or.inl rfl) ha hd
      (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kd e1 e4 1
...
lemma dispatch_bd12G
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q) (hq4m : q % 4 = 1)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    (he3 : e3 = 1 ∨ e3 = -1) (he4 : e4 = 1 ∨ e4 = -1)
    (hbd : (Kb = M8 A B C D ∧ Kd = M9 A B C D) ∨ (Kb = M9 A B C D ∧ Kd = M8 A B C D))
    (ha : Ka = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ Ka = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ Ka = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ Ka = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  · exact cross_pair_core_S12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kc e1 e3 1
      he1 he3 (Or.inl rfl) ha hc (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_T12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kc e1 e3 1
      he1 he3 (Or.inl rfl) ha hc (by linarith [hE1, hE2]) (by linarith [hE1, hE2])
  · exact cross_pair_core_T12 p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kc (-e1) (-e3) 1
      (by rcases he1 with rfl | rfl <;> norm_num)
...
/-- Split a 10-class value for the ratio layer: p²·(F-coordinate class) or
one of the three level-12 classes. -/
lemma ratio_splitG {K : ℤ} (A B C D : ℤ)
    (h : K = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ K = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ K = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ K = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
  ...
  · exact Or.inl ⟨(p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im), (by ring), Or.inl rfl⟩
  · exact Or.inl ⟨(p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im), (by ring), Or.inr (Or.inl rfl)⟩
  · exact Or.inl ⟨(q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im), (by ring), Or.inr (Or.inr (Or.inl rfl))⟩
  · exact Or.inl ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)), (by ring), Or.inr (Or.inr (Or.inr (Or.inl rfl)))⟩
  · exact Or.inl ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)), (by ring), Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))⟩
  · exact Or.inl ⟨((((⟨A, B⟩ : GaussianInt) ^ 8) * ((⟨C, D⟩ : GaussianInt) ^ 4)).im), (by simp only [Zsqrtd.im_mul]; try ring), Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))⟩
  · exact Or.inl ⟨((((⟨A, B⟩ : GaussianInt) ^ 8) * ((star (⟨C, D⟩ : GaussianInt)) ^ 4)).im), (by simp only [Zsqrtd.im_mul, ← star_pow, Zsqrtd.re_star, Zsqrtd.im_star]; try ring), Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (rfl))))))⟩
  · exact Or.inr (Or.inl rfl)
  · exact Or.inr (Or.inr (Or.inl rfl))
  · exact Or.inr (Or.inr (Or.inr rfl))

/-- R₁₂Y + ε·I₁₂X never vanishes. -/
lemma M8v_ne_zero
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (ε : ℤ) (hε : ε = 1 ∨ ε = -1)
    (h : (((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + ε * ((((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) = 0) : False := by
  ...
  have hp12c : (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 + (((⟨A, B⟩ : GaussianInt) ^ 12).im) ^ 2 = (p : ℤ) ^ 12 := norm12_coord p A B hpAB
  have hIY : (((⟨A, B⟩ : GaussianInt) ^ 12).im) ∣ (((⟨C, D⟩ : GaussianInt) ^ 4).im) := by
    have hd : (((⟨A, B⟩ : GaussianInt) ^ 12).im) ∣ (((⟨C, D⟩ : GaussianInt) ^ 4).im) * (((⟨A, B⟩ : GaussianInt) ^ 12).re) := ⟨-(ε * (((⟨C, D⟩ : GaussianInt) ^ 4).re)), by linear_combination h⟩
    exact (hcop.symm).dvd_of_dvd_mul_right hd
  obtain ⟨u, hu⟩ := hIY
  have hX : ε * (((⟨C, D⟩ : GaussianInt) ^ 4).re) = -((((⟨A, B⟩ : GaussianInt) ^ 12).re) * u) := by
    have h0 : (((⟨A, B⟩ : GaussianInt) ^ 12).im) * ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * u + ε * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) = 0 := by
      linear_combination h - (((⟨A, B⟩ : GaussianInt) ^ 12).re) * hu
    rcases mul_eq_zero.mp h0 with h' | h'
    · exact absurd h' hI120
    · linarith
  have hXsq : (ε * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) ^ 2 = (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 * u ^ 2 := by rw [hX]; ring
  have hX2 : (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 = (((⟨A, B⟩ : GaussianInt) ^ 12).re) ^ 2 * u ^ 2 := by
    linear_combination hXsq - (((⟨C, D⟩ : GaussianInt) ^ 4).re) ^ 2 * hε2
  have hY2 : (((⟨C, D⟩ : GaussianInt) ^ 4).im) ^ 2 = (((⟨A, B⟩ : GaussianInt) ^ 12).im) ^ 2 * u ^ 2 := by rw [hu]; ring
  have hkey : u ^ 2 * (p : ℤ) ^ 12 = (q : ℤ) ^ 4 := by
    linear_combination hq4c - hX2 - hY2 - u ^ 2 * hp12c
  have hdq : (p : ℤ) ∣ (q : ℤ) ^ 4 :=
    (dvd_pow_self _ (by norm_num : (12 : ℕ) ≠ 0)).trans ⟨u ^ 2, by linear_combination -hkey⟩
  have hq' : (p : ℤ) ∣ (q : ℤ) := hpP.dvd_of_dvd_pow hdq
  have hnat : p ∣ q := by exact_mod_cast hq'
  exact hpq ((Nat.prime_dvd_prime_iff_eq hp.out hq.out).mp hnat)

/-- χ does not divide the conjugate of π¹²·χ⁴. -/
lemma chi_not_dvd_star_p12q4
    (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q) :
    ¬ (⟨C, D⟩ : GaussianInt) ∣
      star (((⟨A, B⟩ : GaussianInt) ^ 12) * ((⟨C, D⟩ : GaussianInt) ^ 4)) := by
...
/-- q² never divides c·(R₁₂Y + I₁₂X) for q ∤ c. -/
lemma q2_not_dvd_M8v
    (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (c m : ℤ) (hc : ¬ (q : ℤ) ∣ c)
    (h : c * ((((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)) = (q : ℤ) ^ 2 * m) : False := by
  refine p2_extract_kill q hqodd C D hqCD
    (((⟨A, B⟩ : GaussianInt) ^ 12) * ((⟨C, D⟩ : GaussianInt) ^ 4)) c m hc
    (Dvd.dvd.mul_left (dvd_pow_self _ (by norm_num)) _)
    (chi_not_dvd_star_p12q4 p q hqodd hpq A B C D hpAB hqCD) ?_
  have hco := M8_coord A B C D
  unfold M8 at hco
  rw [hco]
  exact h

/-- q² never divides c·(I₁₂X − R₁₂Y) for q ∤ c. -/
lemma q2_not_dvd_M9v
    (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (c m : ℤ) (hc : ¬ (q : ℤ) ∣ c)
    (h : c * ((((⟨A, B⟩ : GaussianInt) ^ 12).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 12).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)) = (q : ℤ) ^ 2 * m) : False := by
  have hstar : (star (⟨C, D⟩ : GaussianInt)) = (⟨C, -D⟩ : GaussianInt) := by ext <;> simp
  have hqCD2 : C ^ 2 + (-D) ^ 2 = q := by rw [neg_pow]; ring_nf; linarith [hqCD]
  refine p2_extract_kill q hqodd C (-D) hqCD2
    (((⟨A, B⟩ : GaussianInt) ^ 12) * ((⟨C, -D⟩ : GaussianInt) ^ 4)) c m hc
    (Dvd.dvd.mul_left (dvd_pow_self _ (by norm_num)) _)
    (chi_not_dvd_star_p12q4 p q hqodd hpq A B C (-D) hpAB hqCD2) ?_
  have hco := M9_coord A B C D
  unfold M9 at hco
  rw [hstar] at hco
  rw [hco]
  exact h
...
set_option maxHeartbeats 1600000 in
/-- No 10-class value is 2δ× another. -/
lemma no_ratio2_sp3q
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (W V δ : ℤ) (hδ : δ = 1 ∨ δ = -1)
    (hW : W = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ W = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ W = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ W = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ W = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ W = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ W = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ W = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im)
      ∨ W = M8 A B C D
      ∨ W = M9 A B C D)
    (hV : V = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
      ∨ V = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
      ∨ V = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
      ∨ V = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ V = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ V = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
      ∨ V = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
      ∨ V = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im)
      ∨ V = M8 A B C D
      ∨ V = M9 A B C D)
    (h : W = 2 * δ * V) : False := by
  have hpP : Prime (p : ℤ) := by
  ...
  have hpnq2 : ¬ (p : ℤ) ∣ ((q : ℤ) ^ 2) := by
    ...
    have hnat : p ∣ q := by exact_mod_cast hq'
    exact hpq ((Nat.prime_dvd_prime_iff_eq hp.out hq.out).mp hnat)
  have hM8c := M8_coord A B C D
  have hM9c := M9_coord A B C D
  have hre6 := re12_eq6 A B
  have him6 := im12_eq6 A B
  have HCXY := coprime_re4_im4 q hqodd C D hqCD
  have HY0 := im4_ne_zero q hqodd C D hqCD
  rcases ratio_splitG p q A B C D hW with ⟨W', rfl, hWm⟩ | hWZ
  · rcases ratio_splitG p q A B C D hV with ⟨V', rfl, hVm⟩ | hVZ
    · have h' : W' = 2 * δ * V' :=
        mul_left_cancel₀ hp20 (by linear_combination h)
      exact no_ratio2_coord p q hpodd hqodd hpq A B C D hpAB hqCD W' V' δ hδ hWm hVm h'
    · rcases hVZ with heq | heq | heq <;> rw [heq] at h
      · exact p2_not_dvd_I12 p hpodd A B hpAB (2 * δ * (q : ℤ) ^ 2) W' hpn2δq
          (by linear_combination -h)
      · exact p2_not_dvd_M8 p q hpodd hpq A B C D hpAB hqCD (2 * δ) W' hpn2δ
          (by linear_combination -h)
      · exact p2_not_dvd_M9 p q hpodd hpq A B C D hpAB hqCD (2 * δ) W' hpn2δ
          (by linear_combination -h)
  · rcases ratio_splitG p q A B C D hV with ⟨V', rfl, hVm⟩ | hVZ
    · rcases hWZ with rfl | rfl | rfl
      · exact p2_not_dvd_I12 p hpodd A B hpAB ((q : ℤ) ^ 2) (2 * δ * V') hpnq2
          (by linear_combination h)
      · exact p2_not_dvd_M8 p q hpodd hpq A B C D hpAB hqCD 1 (2 * δ * V') hpn1
          (by linear_combination h)
      · exact p2_not_dvd_M9 p q hpodd hpq A B C D hpAB hqCD 1 (2 * δ * V') hpn1
          (by linear_combination h)
    · rcases hWZ with rfl | rfl | rfl <;> rcases hVZ with heq | heq | heq <;> rw [heq] at h
      · -- M7 = 2δ·M7
        have h0 : (q : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12).im) * (1 - 2 * δ)) = 0 := by linear_combination h
        rcases mul_eq_zero.mp h0 with h' | h'
        · exact absurd h' hq20
        · rcases mul_eq_zero.mp h' with h'' | h''
          · exact hI120 h''
          · rcases hδ with rfl | rfl <;> omega
      · exact q2_not_dvd_M8v p q hqodd hpq A B C D hpAB hqCD (2 * δ) ((((⟨A, B⟩ : GaussianInt) ^ 12).im)) hqn2δ
          (by linear_combination -h - 2 * δ * hM8c)
      · exact q2_not_dvd_M9v p q hqodd hpq A B C D hpAB hqCD (2 * δ) ((((⟨A, B⟩ : GaussianInt) ^ 12).im)) hqn2δ
          (by linear_combination -h - 2 * δ * hM9c)
      · exact q2_not_dvd_M8v p q hqodd hpq A B C D hpAB hqCD 1 (2 * δ * (((⟨A, B⟩ : GaussianInt) ^ 12).im)) hqn1
          (by linear_combination h - hM8c)
      · -- M8 = 2δ·M8
        refine M8v_ne_zero p q hpodd hqodd hpq A B C D hpAB hqCD 1 (Or.inl rfl) ?_
        have h0 : M8 A B C D * (1 - 2 * δ) = 0 := by linear_combination h
        rcases mul_eq_zero.mp h0 with h' | h'
        · linear_combination h' - hM8c
        · exfalso; rcases hδ with rfl | rfl <;> omega
      · -- M8 = 2δ·M9
        exact ratio_M8_M9_kill p q hpodd hqodd hpq
          (((⟨A, B⟩ : GaussianInt) ^ 6).re) (((⟨A, B⟩ : GaussianInt) ^ 6).im) ((((⟨C, D⟩ : GaussianInt) ^ 4).re)) ((((⟨C, D⟩ : GaussianInt) ^ 4).im)) δ hδ
          (norm6_coord p A B hpAB) (norm4_coord q C D hqCD)
          (coprime_re6_im6 p hpodd A B hpAB) HCXY
          (re6_odd p hpodd A B hpAB)
          (re6_ne_zero p hpodd A B hpAB) (im6_ne_zero p hpodd A B hpAB)
          (odd_ne_zero (re4_odd' q hqodd C D hqCD)) HY0
          (by linear_combination h - hM8c + 2 * δ * hM9c
            - (1 + 2 * δ) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) * hre6 - (1 - 2 * δ) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) * him6)
      · exact q2_not_dvd_M9v p q hqodd hpq A B C D hpAB hqCD 1 (2 * δ * (((⟨A, B⟩ : GaussianInt) ^ 12).im)) hqn1
          (by linear_combination h - hM9c)
      · -- M9 = 2δ·M8 : Y ↦ −Y
        exact ratio_M8_M9_kill p q hpodd hqodd hpq
          (((⟨A, B⟩ : GaussianInt) ^ 6).re) (((⟨A, B⟩ : GaussianInt) ^ 6).im) ((((⟨C, D⟩ : GaussianInt) ^ 4).re)) (-(((⟨C, D⟩ : GaussianInt) ^ 4).im)) δ hδ
          (norm6_coord p A B hpAB) (by linear_combination norm4_coord q C D hqCD)
          (coprime_re6_im6 p hpodd A B hpAB) (HCXY.neg_right)
          (re6_odd p hpodd A B hpAB)
          (re6_ne_zero p hpodd A B hpAB) (im6_ne_zero p hpodd A B hpAB)
          ...
          (by linear_combination h - hM9c + 2 * δ * hM8c
            + (1 + 2 * δ) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) * hre6 - (1 - 2 * δ) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) * him6)
      · -- M9 = 2δ·M9
        refine M8v_ne_zero p q hpodd hqodd hpq A B C D hpAB hqCD (-1) (Or.inr rfl) ?_
        have h0 : M9 A B C D * (1 - 2 * δ) = 0 := by linear_combination h
        rcases mul_eq_zero.mp h0 with h' | h'
        · linear_combination hM9c - h'
        · exfalso; rcases hδ with rfl | rfl <;> omega

set_option maxHeartbeats 1600000 in
/-- Four differences u, v, u+v, u−v cannot all lie in D(s·p³·q). -/
theorem no_four_diffs_sp3q
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (s : ℕ) (hs : ∀ r : ℕ, r.Prime → r ∣ s → r % 4 ≠ 1) (hs0 : 0 < s)
    (u v : ℤ) (hu0 : u ≠ 0) (hv0 : v ≠ 0) (huv : u ≠ v) (huv' : u ≠ -v)
    (x1 y1 x2 y2 x3 y3 x4 y4 : ℤ)
    (h1 : x1 ^ 2 + y1 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 6 * (q : ℤ) ^ 2) (e1 : 2 * x1 * y1 = u)
    (h2 : x2 ^ 2 + y2 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 6 * (q : ℤ) ^ 2) (e2 : 2 * x2 * y2 = v)
    (h3 : x3 ^ 2 + y3 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 6 * (q : ℤ) ^ 2) (e3 : 2 * x3 * y3 = u + v)
    (h4 : x4 ^ 2 + y4 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 6 * (q : ℤ) ^ 2) (e4 : 2 * x4 * y4 = u - v) :
    False := by
  have hq4m : q % 4 = 1 := by
    have hqz : (q : ℤ) % 4 = 1 := by
      rcases Int.even_or_odd C with ⟨c, hc⟩ | ⟨c, hc⟩ <;>
        ...
        rw [hc, hd] at hqCD
      · exfalso
        obtain ⟨m, hm⟩ := odd_cast q hqodd
        rw [hm] at hqCD
        have hx : 4 * (c * c) + 4 * (d * d) = 2 * m + 1 := by linear_combination hqCD
        generalize c * c = c2 at hx
        generalize d * d = d2 at hx
        omega
      · have hx : (q : ℤ) = 4 * (c * c + d * d + d) + 1 := by linear_combination -hqCD
        omega
      · have hx : (q : ℤ) = 4 * (c * c + c + d * d) + 1 := by linear_combination -hqCD
        omega
      · exfalso
        obtain ⟨m, hm⟩ := odd_cast q hqodd
        rw [hm] at hqCD
        have hx : 4 * (c * c + c + d * d + d) + 2 = 2 * m + 1 := by linear_combination hqCD
        generalize c * c = c2 at hx
        generalize d * d = d2 at hx
        omega
    omega
  obtain ⟨ε1, hε1, hd1⟩ := rep_structure_sp3q p q hpq A B C D hpAB hqCD s hs x1 y1 h1
    (by rw [e1]; exact hu0)
  obtain ⟨ε2, hε2, hd2⟩ := rep_structure_sp3q p q hpq A B C D hpAB hqCD s hs x2 y2 h2
    (by rw [e2]; exact hv0)
  obtain ⟨ε3, hε3, hd3⟩ := rep_structure_sp3q p q hpq A B C D hpAB hqCD s hs x3 y3 h3
    (by rw [e3]; intro h; exact huv' (by linarith))
  obtain ⟨ε4, hε4, hd4⟩ := rep_structure_sp3q p q hpq A B C D hpAB hqCD s hs x4 y4 h4
    (by rw [e4]; intro h; exact huv (by linarith))
  rw [e1] at hd1; rw [e2] at hd2; rw [e3] at hd3; rw [e4] at hd4
  have him84 : (((⟨A, B⟩ : GaussianInt) ^ 8 * (⟨C, D⟩ : GaussianInt) ^ 4).im) = (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) := by
    simp [Zsqrtd.im_mul]
  have him84s : (((⟨A, B⟩ : GaussianInt) ^ 8 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im) = (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) := by
    simp only [Zsqrtd.im_mul, ← star_pow, Zsqrtd.re_star, Zsqrtd.im_star]
    ring
  have pack : ∀ w εw,
      (w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 4 * (((⟨A, B⟩ : GaussianInt) ^ 4 * (⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 4 * (((⟨A, B⟩ : GaussianInt) ^ 4 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8 * (⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12 * (⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 12 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im)))) →
      ∃ K, (K = (p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
        ∨ K = (p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
        ∨ K = (p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im))
        ∨ K = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
        ∨ K = (p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
        ...
        ∧ w = εw * ((s : ℤ) ^ 2 * K) := by
    intro w εw hw
    rcases hw with h | h | h | h | h | h | h | h | h | h
    · exact ⟨(p : ℤ) ^ 6 * (((⟨C, D⟩ : GaussianInt) ^ 4).im), Or.inl rfl, h⟩
    · exact ⟨(p : ℤ) ^ 4 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im), Or.inr (Or.inl rfl), h⟩
    · refine ⟨(p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)), Or.inr (Or.inr (Or.inr (Or.inl rfl))), ?_⟩
      rw [im_mul44] at h
      exact h
    · refine ⟨(p : ℤ) ^ 4 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)), Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))), ?_⟩
      rw [im_mul44s] at h
      exact h
    · exact ⟨(p : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)), Or.inr (Or.inr (Or.inl rfl)), by linear_combination h⟩
    · refine ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re)), Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))), ?_⟩
      rw [him84] at h
      exact h
    · refine ⟨(p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 8).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 8).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im)), Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))), ?_⟩
      rw [him84s] at h
      exact h
    · exact ⟨(q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 12).im), Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))), h⟩
    · refine ⟨M8 A B C D, Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))))), ?_⟩
      unfold M8
      exact h
    · refine ⟨M9 A B C D, Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (rfl))))))))), ?_⟩
      unfold M9
      exact h
  ...
  have hcancel : ∀ Kx Ky d : ℤ, (s : ℤ) ^ 2 * (Kx - 2 * d * Ky) = 0 → Kx = 2 * d * Ky := by
    intro Kx Ky d h0
    rcases mul_eq_zero.mp h0 with h' | h'
    · exact absurd h' hs2
    · linarith
  -- distinctness of the four class values
  have hab : Ka ≠ Kb := by
    intro he
    rw [he] at hu
    rcases hε1 with rfl | rfl <;> rcases hε2 with rfl | rfl
    · exact huv (by linarith [hu, hv])
    · exact huv' (by linarith [hu, hv])
    · exact huv' (by linarith [hu, hv])
    · exact huv (by linarith [hu, hv])
  have hcd : Kc ≠ Kd := by
    ...
    · exact hv0 (by linarith [hsv, hdv])
    · exact hu0 (by linarith [hsv, hdv])
    · exact hu0 (by linarith [hsv, hdv])
    · exact hv0 (by linarith [hsv, hdv])
  have hac : Ka ≠ Kc := by
    intro he
    rw [he] at hu
    rcases hε1 with rfl | rfl <;> rcases hε3 with rfl | rfl
    · exact hv0 (by linarith [hu, hsv])
    · rcases hε2 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kc (-1)
          (Or.inr rfl) hKb hKc (hcancel Kb Kc (-1) (by linear_combination hsv - hu - hv))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kc (1)
          (Or.inl rfl) hKb hKc (hcancel Kb Kc (1) (by linear_combination hu + hv - hsv))
    · rcases hε2 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kc (1)
          (Or.inl rfl) hKb hKc (hcancel Kb Kc (1) (by linear_combination hsv - hu - hv))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kc (-1)
          (Or.inr rfl) hKb hKc (hcancel Kb Kc (-1) (by linear_combination hu + hv - hsv))
    · exact hv0 (by linarith [hu, hsv])
  have had : Ka ≠ Kd := by
    intro he
    rw [he] at hu
    rcases hε1 with rfl | rfl <;> rcases hε4 with rfl | rfl
    · exact hv0 (by linarith [hu, hdv])
    · rcases hε2 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kd (1)
          (Or.inl rfl) hKb hKd (hcancel Kb Kd (1) (by linear_combination hu - hdv - hv))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kd (-1)
          (Or.inr rfl) hKb hKd (hcancel Kb Kd (-1) (by linear_combination hdv + hv - hu))
    · rcases hε2 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kd (-1)
          (Or.inr rfl) hKb hKd (hcancel Kb Kd (-1) (by linear_combination hu - hdv - hv))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kd (1)
          (Or.inl rfl) hKb hKd (hcancel Kb Kd (1) (by linear_combination hdv + hv - hu))
    · exact hv0 (by linarith [hu, hdv])
  have hbc : Kb ≠ Kc := by
    intro he
    rw [he] at hv
    rcases hε2 with rfl | rfl <;> rcases hε3 with rfl | rfl
    · exact hu0 (by linarith [hv, hsv])
    · rcases hε1 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kc (-1)
          (Or.inr rfl) hKa hKc (hcancel Ka Kc (-1) (by linear_combination hsv - hv - hu))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kc (1)
          (Or.inl rfl) hKa hKc (hcancel Ka Kc (1) (by linear_combination hv + hu - hsv))
    · rcases hε1 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kc (1)
          (Or.inl rfl) hKa hKc (hcancel Ka Kc (1) (by linear_combination hsv - hv - hu))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kc (-1)
          (Or.inr rfl) hKa hKc (hcancel Ka Kc (-1) (by linear_combination hv + hu - hsv))
    · exact hu0 (by linarith [hv, hsv])
  have hbd : Kb ≠ Kd := by
    intro he
    rw [he] at hv
    rcases hε2 with rfl | rfl <;> rcases hε4 with rfl | rfl
    · rcases hε1 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kd (1)
          (Or.inl rfl) hKa hKd (hcancel Ka Kd (1) (by linear_combination hv + hdv - hu))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kd (-1)
          (Or.inr rfl) hKa hKd (hcancel Ka Kd (-1) (by linear_combination hu - hdv - hv))
    · exact hu0 (by linarith [hv, hdv])
    · exact hu0 (by linarith [hv, hdv])
    · rcases hε1 with rfl | rfl
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kd (-1)
          (Or.inr rfl) hKa hKd (hcancel Ka Kd (-1) (by linear_combination hv + hdv - hu))
      · exact no_ratio2_sp3q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kd (1)
          (Or.inl rfl) hKa hKd (hcancel Ka Kd (1) (by linear_combination hu - hdv - hv))
  ...
  have hE2 : ε3 * Kc - ε4 * Kd = 2 * ε2 * Kb := by
    ...
    · exact absurd h' hs2
    · linarith
  exact no_assignment_sp3q p q hpodd hqodd hpq hq4m A B C D hpAB hqCD Ka Kb Kc Kd
    ε1 ε2 ε3 ε4 hε1 hε2 hε3 hε4 hKa hKb hKc hKd hab hac had hbc hbd hcd hE1 hE2

/-- **Theorem G, machine-checked.** No 3×3 fully magic square of nine
perfect squares has center entry (s·p³·q)² for distinct primes
p, q ≡ 1 (mod 4) and rigid s. -/
theorem no_magic_square_of_squares_sp3q_center
  ...
  have he2 : e ^ 2 = ((s : ℕ) : ℤ) ^ 2 * (p : ℤ) ^ 6 * (q : ℤ) ^ 2 := by omega
  ...
  have huv' : u ≠ -v := by omega
  exact no_four_diffs_sp3q p q (by omega) (by omega) hpq
    (PA : ℤ) (PB : ℤ) (QC : ℤ) (QD : ℤ)
