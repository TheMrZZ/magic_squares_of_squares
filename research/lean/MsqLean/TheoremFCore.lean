...
lemma descent_norm4' (a b : ℕ)
    (haodd : a % 2 = 1) (hbodd : b % 2 = 1) (hcopab : Nat.Coprime a b) (hab : a ≠ b)
    (ha0 : 0 < a) (hb0 : 0 < b)
    (T : ℤ) (h : (a : ℤ) ^ 4 - (b : ℤ) ^ 4 = 8 * T ^ 2) : False := by
  obtain ⟨da, hda⟩ := odd_cast a haodd
  obtain ⟨db, hdb⟩ := odd_cast b hbodd
  ...
  have hcopAB : IsCoprime A B' := by
    ...
    have hra' : r ∣ a := by exact_mod_cast hra
    have hrb' : r ∣ b := by exact_mod_cast hrb
    have hr1 : r ∣ 1 := hcopab ▸ Nat.dvd_gcd hra' hrb'
    have := Nat.le_of_dvd (by norm_num) hr1
    have := hrprime.two_le
    omega
  have hApos : 0 < A := by
    have ha0' : (0 : ℤ) < (a : ℤ) := by exact_mod_cast ha0
    have hb0' : (0 : ℤ) < (b : ℤ) := by exact_mod_cast hb0
    nlinarith [hAeq, ha0', hb0']
  obtain ⟨u, hu⟩ := Int.sq_of_isCoprime hcopAB hT2
  have hAu : A = u ^ 2 := by
    rcases hu with h' | h'
...
set_option maxHeartbeats 3200000 in
/-- Deep ratio cell (L5 = ±2·L6 coordinates): the level-8 chains land
on q⁴ vs p⁸ norm systems, dead by the coprime Fermat descent. -/
lemma ratio_L5_L6_kill
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (R I X Y δ : ℤ) (hδ : δ = 1 ∨ δ = -1)
    (hp4 : R ^ 2 + I ^ 2 = (p : ℤ) ^ 4)
    (hq4 : X ^ 2 + Y ^ 2 = (q : ℤ) ^ 4)
  ...
  have hnorm8 : (R ^ 2 - I ^ 2) ^ 2 + (2 * R * I) ^ 2 = (p : ℤ) ^ 8 := by
    have hp8 : (R ^ 2 + I ^ 2) ^ 2 = (p : ℤ) ^ 8 := by rw [hp4]; ring
    linear_combination hp8
  have hp2odd : p ^ 2 % 2 = 1 := by
    have := Nat.pow_mod p 2 2
    rw [hpodd] at this
    simpa using this
  have hcopqp2 : Nat.Coprime q (p ^ 2) :=
    Nat.Coprime.pow_right 2 ((Nat.coprime_primes hq.out hp.out).mpr (Ne.symm hpq))
  have hqnep2 : q ≠ p ^ 2 := by
    intro hqe
    have hpd : p ∣ q := by rw [hqe]; exact ⟨p, by ring⟩
    have hpe := (Nat.prime_dvd_prime_iff_eq hp.out hq.out).mp hpd
    rw [← hpe] at hqe
    nlinarith [hp.out.two_le, hqe]
  have hq0 : 0 < q := hq.out.pos
  have hp20 : 0 < p ^ 2 := pow_pos hp.out.pos 2
  have hcast : (((p ^ 2 : ℕ)) : ℤ) ^ 4 = (p : ℤ) ^ 8 := by push_cast; ring
  rcases hδ with rfl | rfl
  · -- 3R₈Y = I₈X
    ...
    · have hsq : (2 * R * I) ^ 2 = Y ^ 2 := by rw [hih]; ring
      exact descent_norm4' q (p ^ 2) hqodd hp2odd hcopqp2 hqnep2 hq0 hp20
        (R ^ 2 - I ^ 2) (by rw [hcast]; linear_combination -hq4 + hnorm8 - hsq)
    · have hsq : (2 * R * I) ^ 2 = Y ^ 2 := by rw [hih]; ring
      exact descent_norm4' q (p ^ 2) hqodd hp2odd hcopqp2 hqnep2 hq0 hp20
        (R ^ 2 - I ^ 2) (by rw [hcast]; linear_combination -hq4 + hnorm8 - hsq)
    · have hsq : (2 * R * I) ^ 2 = 9 * Y ^ 2 := by rw [hih]; ring
      exact descent_norm4' (p ^ 2) q hp2odd hqodd hcopqp2.symm (Ne.symm hqnep2) hp20 hq0
...
set_option maxHeartbeats 3200000 in
/-- No four differences u, v, u±v can all be twice-products at norm
s²p⁴q². Packs the seven classes, derives the pairwise distinctness of
the class values (via no_ratio2 for the ±2u branches), cancels s²,
and routes to the assignment router. -/
theorem no_four_diffs_sp2q
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (s : ℕ) (hs : ∀ r : ℕ, r.Prime → r ∣ s → r % 4 ≠ 1) (hs0 : 0 < s)
    (u v : ℤ) (hu0 : u ≠ 0) (hv0 : v ≠ 0) (huv : u ≠ v) (huv' : u ≠ -v)
    (x1 y1 x2 y2 x3 y3 x4 y4 : ℤ)
    (h1 : x1 ^ 2 + y1 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 4 * (q : ℤ) ^ 2) (e1 : 2 * x1 * y1 = u)
    (h2 : x2 ^ 2 + y2 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 4 * (q : ℤ) ^ 2) (e2 : 2 * x2 * y2 = v)
    (h3 : x3 ^ 2 + y3 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 4 * (q : ℤ) ^ 2) (e3 : 2 * x3 * y3 = u + v)
    (h4 : x4 ^ 2 + y4 ^ 2 = (s : ℤ) ^ 2 * (p : ℤ) ^ 4 * (q : ℤ) ^ 2) (e4 : 2 * x4 * y4 = u - v) :
    False := by
  obtain ⟨ε1, hε1, hd1⟩ := rep_structure_sp2q p q hpq A B C D hpAB hqCD s hs x1 y1 h1
    (by rw [e1]; exact hu0)
  obtain ⟨ε2, hε2, hd2⟩ := rep_structure_sp2q p q hpq A B C D hpAB hqCD s hs x2 y2 h2
    (by rw [e2]; exact hv0)
  obtain ⟨ε3, hε3, hd3⟩ := rep_structure_sp2q p q hpq A B C D hpAB hqCD s hs x3 y3 h3
    (by rw [e3]; intro h; exact huv' (by linarith))
  obtain ⟨ε4, hε4, hd4⟩ := rep_structure_sp2q p q hpq A B C D hpAB hqCD s hs x4 y4 h4
    (by rw [e4]; intro h; exact huv (by linarith))
  rw [e1] at hd1; rw [e2] at hd2; rw [e3] at hd3; rw [e4] at hd4
  have pack : ∀ w εw,
      (w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4 * (⟨C, D⟩ : GaussianInt) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * ((p : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im)))
       ∨ w = εw * ((s : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8 * (⟨C, D⟩ : GaussianInt) ^ 4).im))
       ∨ w = εw * ((s : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8 * (star (⟨C, D⟩ : GaussianInt)) ^ 4).im))) →
      ∃ K, (K = L0 p C D ∨ K = L1 p q A B ∨ K = L2 q A B
        ∨ K = L3 p A B C D ∨ K = L4 p A B C D ∨ K = L5 A B C D ∨ K = L6 A B C D)
        ∧ w = εw * ((s : ℤ) ^ 2 * K) := by
    intro w εw hw
    rcases hw with h | h | h | h | h | h | h
    · exact ⟨L0 p C D, Or.inl rfl, by unfold L0; exact h⟩
    · exact ⟨L1 p q A B, Or.inr (Or.inl rfl), by unfold L1; exact h⟩
    · exact ⟨L2 q A B, Or.inr (Or.inr (Or.inl rfl)), by unfold L2; exact h⟩
    · refine ⟨L3 p A B C D, Or.inr (Or.inr (Or.inr (Or.inl rfl))), ?_⟩
      unfold L3
      rw [im_mul44] at h
      exact h
    · refine ⟨L4 p A B C D, Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))), ?_⟩
      unfold L4
      rw [im_mul44s] at h
      exact h
    · exact ⟨L5 A B C D, Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))),
        by unfold L5; exact h⟩
    · exact ⟨L6 A B C D, Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr rfl))))),
        by unfold L6; exact h⟩
  ...
  have hac : Ka ≠ Kc := by
    ...
    · rcases hε2 with rfl | rfl
      · exact no_ratio2 p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kc (-1)
  ...
  have had : Ka ≠ Kd := by
    ...
    · rcases hε2 with rfl | rfl
      · exact no_ratio2 p q hpodd hqodd hpq A B C D hpAB hqCD Kb Kd (1)
  ...
  have hbc : Kb ≠ Kc := by
    ...
    · rcases hε1 with rfl | rfl
      · exact no_ratio2 p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kc (-1)
  ...
  have hbd : Kb ≠ Kd := by
    ...
    · rcases hε1 with rfl | rfl
      · exact no_ratio2 p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kd (1)
  ...
  have hE2 : ε3 * Kc - ε4 * Kd = 2 * ε2 * Kb := by
    ...
    · exact absurd h' hs2
    · linarith
  exact no_assignment_sp2q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kb Kc Kd
...
/-- Public coordinate form of the s·p²·q assignment router, for reuse
by the s·p³·q layer after its p²-cancellation. -/
lemma no_assignment_sp2q_coord
    (hpodd : p % 2 = 1) (hqodd : q % 2 = 1) (hpq : p ≠ q)
    (A B C D : ℤ) (hpAB : A ^ 2 + B ^ 2 = p) (hqCD : C ^ 2 + D ^ 2 = q)
    (Ka Kb Kc Kd e1 e2 e3 e4 : ℤ)
    (he1 : e1 = 1 ∨ e1 = -1) (he2 : e2 = 1 ∨ e2 = -1)
    (he3 : e3 = 1 ∨ e3 = -1) (he4 : e4 = 1 ∨ e4 = -1)
    (ha : Ka = (p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
        ∨ Ka = (p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
        ∨ Ka = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)
        ∨ Ka = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
        ∨ Ka = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
        ∨ Ka = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((⟨C, D⟩ : GaussianInt) ^ 4)).im)
        ∨ Ka = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((star (⟨C, D⟩ : GaussianInt)) ^ 4)).im))
    (hb : Kb = (p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
        ∨ Kb = (p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
        ∨ Kb = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)
        ∨ Kb = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
        ∨ Kb = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
        ∨ Kb = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((⟨C, D⟩ : GaussianInt) ^ 4)).im)
        ∨ Kb = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((star (⟨C, D⟩ : GaussianInt)) ^ 4)).im))
    (hc : Kc = (p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
        ∨ Kc = (p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
        ∨ Kc = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)
        ∨ Kc = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
        ∨ Kc = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
        ∨ Kc = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((⟨C, D⟩ : GaussianInt) ^ 4)).im)
        ∨ Kc = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((star (⟨C, D⟩ : GaussianInt)) ^ 4)).im))
    (hd : Kd = (p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
        ∨ Kd = (p : ℤ) ^ 2 * (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 4).im)
        ∨ Kd = (q : ℤ) ^ 2 * (((⟨A, B⟩ : GaussianInt) ^ 8).im)
        ∨ Kd = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im) + (((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re))
        ∨ Kd = (p : ℤ) ^ 2 * ((((⟨A, B⟩ : GaussianInt) ^ 4).im) * (((⟨C, D⟩ : GaussianInt) ^ 4).re) - (((⟨A, B⟩ : GaussianInt) ^ 4).re) * (((⟨C, D⟩ : GaussianInt) ^ 4).im))
        ∨ Kd = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((⟨C, D⟩ : GaussianInt) ^ 4)).im)
        ∨ Kd = ((((⟨A, B⟩ : GaussianInt) ^ 8) * ((star (⟨C, D⟩ : GaussianInt)) ^ 4)).im))
    (hab : Ka ≠ Kb) (hac : Ka ≠ Kc) (had : Ka ≠ Kd)
    (hbc : Kb ≠ Kc) (hbd : Kb ≠ Kd) (hcd : Kc ≠ Kd)
    (hE1 : e3 * Kc + e4 * Kd = 2 * e1 * Ka)
    (hE2 : e3 * Kc - e4 * Kd = 2 * e2 * Kb) : False := by
  have conv : ∀ K : ℤ, (K = (p : ℤ) ^ 4 * (((⟨C, D⟩ : GaussianInt) ^ 4).im)
    ...
    · exact Or.inl (by unfold L0; exact h)
    · exact Or.inr (Or.inl (by unfold L1; exact h))
  ...
  exact no_assignment_sp2q p q hpodd hqodd hpq A B C D hpAB hqCD Ka Kb Kc Kd
    e1 e2 e3 e4 he1 he2 he3 he4 (conv Ka ha) (conv Kb hb) (conv Kc hc) (conv Kd hd)
