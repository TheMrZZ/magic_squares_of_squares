...
from chi2_engine_lib import build, gauss_rep, cpow, cpow, eval_gauss_polyQ, norm2_polyQ
    ...
    for p in primes:
        ...
        for js in itertools.product(range(1,7), repeat=nlev):
            ...
            if cr == 0:
                ...
                depth = v - emin
                depths.append(depth)
                # LTE prediction when minimal set is a +-1 pair
                mins_pairs = [(B2, gr2) for P2, B2, gr2 in live
                              if sum(P2[k]*js[k] for k in range(nlev)) == emin]
                if len(mins_pairs) == 2 and sorted(g for _, g in mins_pairs) == [-1, 1]:
                    b1 = sum(mins_pairs[0][0][k]*js[k] for k in range(nlev))
                    b2 = sum(mins_pairs[1][0][k]*js[k] for k in range(nlev))
                    Db = abs(b1 - b2)
                    wb = cpow((Ag, -Bg), Db); w = (wb[0]-1, wb[1])
                    Nw = w[0]*w[0] + w[1]*w[1]
                    vv = 0
                    while Nw and Nw % p == 0: Nw //= p; vv += 1
                    lte_cmp.append((depth, vv, depth == vv))
...
print("LTE comparisons:", len(lte_cmp), "; matches:", sum(1 for _,_,m in lte_cmp if m))
print("mismatch examples:", [x for x in lte_cmp if not x[2]][:10])
