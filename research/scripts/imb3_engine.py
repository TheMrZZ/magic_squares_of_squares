"""3-level imbalance sweep: for every 3-level leaf, check the pi/pibar
...
Z1, Z1b, Z2, Z2b, Z3, Z3b = symbols('Z1 Z1b Z2 Z2b Z3 Z3b')  # level orbits: pi^{4j1}, chains
...
VARS = (Z1, Z1b, Z2, Z2b, Z3, Z3b, N1v, N2v, N3v)
def vforms(e, at_pi=True):
    pol = Poly(expand(e), *VARS)
    out = []
    for m in pol.as_dict():
        z1,z1b,z2,z2b,z3,z3b,n1,n2,n3 = m
        if at_pi: out.append((4*z1+2*n1, 4*z2+2*n2, 4*z3+2*n3))
        else: out.append((4*z1b+2*n1, 4*z2b+2*n2, 4*z3b+2*n3))
    return out
...
def residual_analyze(RBv, IBv):
    ...
    def wf(m, pib):
        return tuple((4*m[2*k+1]+2*m[2*3+k]) if pib else (4*m[2*k]+2*m[2*3+k]) for k in range(3))
...
for eu, ev, es, ed in itertools.permutations(names, 4):
    ...
    for signs in itertools.product((1,-1), repeat=len(sn)):
        ...
        if count % 1000 == 0:
            ...
            pickle.dump(fails, open('imb3_fails.pkl','wb'))
        if det == 0:
        ...
        nzero_b = sum(1 for v in vpib if v == (0,0,0))
        nzero_p = sum(1 for v in vpi if v == (0,0,0))
