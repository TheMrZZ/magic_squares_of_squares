...
alive2 = pickle.load(open('sym22_alive2.pkl','rb'))          # 160 alive after unit2
dead_unit2_alive = set(rel for rel, fails in alive2)
prof = pickle.load(open('sym22_profile.pkl','rb'))
# recompute bounded set from sym_bounds logic: reuse its result implicitly:
# bounded relations = FINITE-CLOSED class; recompute classification here
finite = set()
for rel, (cps, cqs) in prof:
    if not cps or cps[0] is None or cqs[0] is None: continue
    hp, hq = cps[0], cqs[0]
    if hp == 0 or hq == 0: continue
    if min(hp, hq) <= 0.5 and max(hp, hq) < 2: finite.add(rel)
print("finite-closed:", len(finite))
all_rels = set(rel for rel, _ in prof)
dead = (all_rels - dead_unit2_alive) | finite
print("total dead relations:", len(dead), "/", len(all_rels))
# leaf coverage
R_, I_, X_, Y_ = symbols('R I X Y')
elems = []
for a in range(3):
    for b in range(3):
        if (a, b) == (0, 0): continue
...
grading_killed = 88832
print(f"TOTAL (2,2) coverage: {grading_killed + deadleaf} / 95040 = {100*(grading_killed+deadleaf)/95040:.2f}%")
