...
def process_pair(item):
    ...
    if not live1 or not live2:
        out_kinds.append('pair dead on recount')
        return out_kinds, out_terms
    for k1 in live1:
        for k2 in live2:
            tag = (k1[0], k2[0])
            if tag == ('AFF', 'AFF'):
                ...
                if j1 != j2_:
                    out_kinds.append('AFF level mismatch')
                    continue
                det = expand(a1*b2 - a2*b1)
                if det == 0:
                    out_kinds.append('AFF parallel')
                    continue
                G1 = expand(-q**(2*e1)*g1)
                G2 = expand(-q**(2*e2)*g2)
                ...
                out_terms.append(str(factor(T)))
                out_kinds.append('AFF-AFF terminal')
            else:
                out_kinds.append(str(tag))
    return out_kinds, out_terms
...
if __name__ == '__main__':
    ...
    with Pool(10) as pool:
        for out_kinds, out_terms in pool.imap_unordered(process_pair, residual, chunksize=8):
            for k in out_kinds: kinds[k] += 1
            for t in out_terms: terms[t] += 1
    print(dict(kinds))
