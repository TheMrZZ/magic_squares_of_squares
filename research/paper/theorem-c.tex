...
\section{Toward center roots $s\,p^2q$}

The next case, $e=s\,p^2q$ --- the type of Bremner's center $425=5^2\cdot17$
--- has $|D(e)|=7$, so the four differences no longer exhaust $D(e)$ and the
valuation screening genuinely matters.  An exact sweep of all placements over
the $1{,}332$ ordered pairs of primes $p,q<400$ finds no configuration.
Analytically, the screening by $\nu_p$ leaves four shape families, and the
first leaves die by the same three mechanisms as Theorem~\ref{thm:minimal}
plus one new one.  For instance, in the shape
$\{u,v\}=\{d_{0,1},d_{1,0}\}$, $\{u\pm v\}=\{d_{1,1},d_{1,-1}\}$, the
conjugate-pair identities force $|R_AR_B|=p^2q^2$ against the strict bounds
$|R_A|<p^2$, $|R_B|<q^2$.  In a deeper leaf one is forced to
$p^2\mid R_B$ together with $r^2(p^4+4I_A^2)=q^4$ for $R_B=p^2r$; then
$p^4+4I_A^2=t^2$ gives $(t-2I_A)(t+2I_A)=p^4$ with $p\nmid I_A$, so
$\{t-2I_A,t+2I_A\}=\{1,p^4\}$ and $I_A=(p^4-1)/4>p^2$, contradicting
$|I_A|\le p^2$.  We expect the remaining leaves to be finite in number and
individually decidable by these mechanisms (positivity pinches, parity,
$p$-adic clashes, and $p$-power factorization against size); completing the
case would prove that no magic square of squares has center root $s\,p^2q$,
eliminating in particular every center of Bremner's type.
